...
class BeforeAnyTaskHookTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @var TaskRunner
     */
    ...
    public function test_arguments_can_be_added_to_context_with_before_hook()
    {
        ...
        $this->sut->beforeAnyTask(function(BeforeTaskContext $context) {
    ...
    public function test_defining_task_after_defining_hook_will_not_overwrite_hook()
    {
        $hookWasRun = false;
        $taskWasRun = false;
        $this->sut->beforeAnyTask(function() use (&$hookWasRun) {
            $hookWasRun = true;
        });
